...
export function AuthProvider({ children }) {
  ...
  // refreshBusiness (app/(app)/layout.js, à chaque navigation). null = pas
  // de blocage lié au paiement, 'en_attente' | 'echoue' = dernier paiement
  // en date, quel que soit business.subscription_status par ailleurs.
  const [paiementBlocage, setPaiementBlocage] = useState(null);
  ...
  // et paiementBlocage doivent toujours être recalculés ensemble. L'appel
  ...
  // await, puis setPaiementBlocage), un rendu intermédiaire affichait
  // business déjà à jour (donc subscription_status='actif', accès
  // autorisé) avec l'ancienne valeur de paiementBlocage — un accès
  ...
  const appliquerBusiness = useCallback(async (result) => {
    const blocage = result && !result.is_admin ? await dernierPaiementBlocage(result.id) : null;
    setBusiness(result);
    setPaiementBlocage(blocage);
  }, []);
