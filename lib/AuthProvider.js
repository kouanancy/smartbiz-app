...
import { dernierPaiementBlocage } from "./paiements";
...
export function AuthProvider({ children }) {
  ...
  // refreshBusiness (app/(app)/layout.js, à chaque navigation). `statut`
  // (null | 'en_attente' | 'echoue') est le dernier paiement de la
  // boutique s'il est en attente ou rejeté, jamais bloquant en lui-même ;
  // `bloquant` dit si l'accès doit réellement être coupé pour ce
  // paiement — jamais vrai si le compte a déjà un accès valide en cours
  // (renouvellement anticipé, voir lib/paiements.js).
  const [paiementInfo, setPaiementInfo] = useState({ statut: null, bloquant: false });
  ...
  // et paiementInfo doivent toujours être recalculés ensemble. L'appel
  // réseau (dernierPaiementBlocage) est résolu AVANT les deux setState,
  // appelés ensuite l'un après l'autre sans await entre eux : React les
  // regroupe alors en un seul rendu. Sans ça (setBusiness immédiat, puis
  // await, puis setPaiementInfo), un rendu intermédiaire affichait
  // business déjà à jour (donc subscription_status='actif', accès
  // autorisé) avec l'ancienne valeur de paiementInfo — un accès
  // temporairement débloqué à chaque connexion/navigation avant que le
  // second setState ne rattrape, le temps d'un aller-retour réseau.
  const appliquerBusiness = useCallback(async (result) => {
    const info = result && !result.is_admin ? await dernierPaiementBlocage(result) : { statut: null, bloquant: false };
    setBusiness(result);
  ...
  const ensureBusiness = useCallback(
    async (user) => {
      if (!user) {
        setBusiness(null);
        setPaiementInfo({ statut: null, bloquant: false });
        return;
      }
      ...
      const task = (async () => {
        ...
        if (existing) {
          console.log("[ensureBusiness] Ligne businesses déjà existante pour ce compte :", existing.id);
          return await verifierExpirationAbonnement(existing);
        }

        console.log("[ensureBusiness] Aucune ligne businesses pour ce compte, création à l'inscription :", user.id);
        ...
        if (!insertError) {
          console.log("[ensureBusiness] Ligne businesses créée avec succès :", created.id);
          return created;
        }

        // Toujours logué en détail (code/message/details/hint Postgres, pas
        // juste "une erreur a eu lieu") : c'est précisément ce qui distingue
        // un doublon inoffensif (23505, voir ci-dessous) d'un vrai blocage
        // RLS/GRANT (ex. 42501 si une colonne du payload n'est plus
        // autorisée par supabase-businesses-colonnes-restreintes-
        // migration.sql) ou d'une contrainte NOT NULL/CHECK.
        console.error("[ensureBusiness] Échec de l'insertion de la ligne businesses à l'inscription :", {
        ...
        if (insertError.code === "23505") {
          console.log("[ensureBusiness] Doublon détecté (23505), récupération de la ligne créée par la requête concurrente.");
