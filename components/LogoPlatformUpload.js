...
export default function LogoPlatformUpload({ label, businessId, value, onChange }) {
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      // (lib/removeWhiteBackground.js) — les icônes carrées ci-dessous,
      // elles, restent générées depuis le fichier opaque d'origine : une
      // icône d'écran d'accueil transparente s'affiche avec un fond noir
      // sur iOS, ce n'est donc pas souhaitable là.
      let logoBlob;
      try {
        const processedCanvas = await removeWhiteBackgroundToCanvas(img);
        logoBlob = await new Promise((resolve, reject) =>
          processedCanvas.toBlob((blob) => (blob ? resolve(blob) : reject(new Error("toBlob a échoué"))), "image/png")
        );
      } catch (err) {
